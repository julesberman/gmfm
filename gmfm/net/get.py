...
def get_arch(net_cfg: Network, out_channels):
    ...
    if net_cfg.arch == "mlp":
        net = get_mlp_size(net_cfg.size, net_cfg.n_harmonics, out_channels)
    return net


def get_mlp_size(size, n_harmonics, out_channels):

    if size == "s":
        net = DNN(width=128, depth=7,
                  out_features=out_channels, residual=False, n_harmonics=n_harmonics)
    elif size == "m":
        net = DNN(width=196, depth=7,
                  out_features=out_channels, residual=False, n_harmonics=n_harmonics)
    elif size == "l":
        net = DNN(width=256, depth=10,
                  out_features=out_channels, residual=True, n_harmonics=n_harmonics)
    return net
