...
def run(cfg: Config) -> None:
    ...
    key, x_data, dataloader, mu, lhs_data, phi_data, loss_fn, net, params_init, apply_fn = build(
        cfg)
...
def build(cfg: Config):
    ...
    mu, lhs_data, phi_data = get_phi_params(cfg, x_data, t_data, p_key)
    dataloader = get_dataloader(cfg, x_data, phi_data, lhs_data, t_data)

    net, apply_fn, params_init = get_network(cfg, dataloader, net_key)

    loss_fn = get_loss_fn(cfg, apply_fn)

    return key, x_data, dataloader, mu, lhs_data, phi_data, loss_fn, net, params_init, apply_fn
